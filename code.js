...
function getProductos() {
  ...
  const hojaPoductos = ss.getSheetByName("productos");

  if (!hojaPoductos) {
    SpreadsheetApp.getUi().alert("No se encontró la hoja 'Poductos'.");
    return;
  }

  // Definir el rango dinámico en la hoja de Poductos
  const ultimaFila = hojaPoductos.getLastRow();
  // Cambiar esto según la columna donde estén los Poductos
  const rangoPoductos = hojaPoductos.getRange(`A2:G${ultimaFila}`); 
  // Obtener los valores del rango y convertirlos en una lista 
  const valoresPoductos = rangoPoductos.getValues(); 
  // Filtrar valores no nulos return
  const listaPoductos = valoresPoductos.flat().filter(producto => producto); 
